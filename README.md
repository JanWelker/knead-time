...
The **Copy TRMNL link** action copies an absolute URL to `/trmnl?…` carrying the current recipe. Paste it into a [TRMNL](https://trmnl.com/) device's **Browser / URL** plugin and the device polls that URL on its usual cadence; the headless renderer hydrates the same client-side bundle, locks the layout to an 800 × 480 box, and snapshots a black-on-white schedule for the e-ink panel. A big **Now / Next** card calls out the active step against the device's clock — past steps fade and strike through, the current step inverts to white-on-black, future steps stay plain — so the same URL stays useful from "soak the biga tonight" all the way to "pizza time".
