...
<form class="space-y-8" onsubmit={(e) => e.preventDefault()}>
	<fieldset class="space-y-3">
		<legend class="font-display text-accent text-lg tracking-[0.02em]">
			{t.form.section_when}
		</legend>
		...
		<fieldset class="group block min-w-0">
			...
			<div class="mt-1 flex items-center justify-between gap-2">
				...
				<button type="button" class="btn-ember-sm ml-auto shrink-0" onclick={resetStartAtToNow}>
					{t.form.startAt_now}
				</button>
			</div>
			{#if startAtClamped}
	...
	<fieldset class="grid grid-cols-1 gap-4 sm:grid-cols-2">
		<legend class="font-display text-accent col-span-full text-lg tracking-[0.02em]">
		...
		{#if uiMode.current === 'expert'}
			<label class="block">
				...
				{#if form.yeastType === 'active-dry'}
					<span class="ink-faint mt-1 block text-xs">
			...
			{:else}
				<fieldset class="space-y-2">
					...
					<label class="ink-soft flex items-center gap-2 text-sm">
						<input
							type="checkbox"
							class="accent-[var(--kt-accent-solid)]"
							bind:checked={form.bigaEnabled}
						/>
					...
					{#if form.bigaEnabled && form.poolishEnabled}
						<span class="ink-faint block text-xs">
			...
			{#if form.yeastType === 'sourdough' || !(form.bigaEnabled || form.poolishEnabled)}
				<label class="ink-soft group flex items-center gap-2 text-sm">
					<input
						type="checkbox"
						class="accent-[var(--kt-accent-solid)]"
						bind:checked={form.autolyse}
					/>
					<span>
						{t.form.autolyse_toggle}
						<span class="ink-faint hidden text-xs font-normal group-focus-within:block">
	...
	{#if uiMode.current === 'expert'}
		<details
			class="ink-soft group min-w-0 rounded-lg border border-dashed border-[var(--kt-line-strong)] bg-[var(--kt-inset)] p-3 text-xs"
		>
			<summary
				...
				{#each INFO_SECTIONS as section (section.title)}
					<div class="min-w-0">
						...
						{#each section.parts as part, i (i)}
							...
							{:else}
								<pre
									class="ink mt-1 overflow-x-auto rounded border border-[var(--kt-line)] bg-[var(--kt-panel)] px-2 py-1 font-mono text-[0.72rem]">{part.formula}</pre>
							{/if}
						{/each}
					</div>
				{/each}
