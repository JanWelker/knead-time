...
<form class="space-y-7" onsubmit={(e) => e.preventDefault()}>
	...
	<!-- A rule between the two groups: "when" and "what" are different
	     questions, and a bare gap did not say so on a long expert form. Drawn as
	     its own element rather than a border on the fieldset, because a browser
	     cuts a notch out of a fieldset's border to seat the legend in it. -->
	<div class="bg-hairline h-px" role="presentation"></div>
	...
	<fieldset class="grid grid-cols-1 gap-4 sm:grid-cols-2">
		...
		{#if uiMode.current === 'expert'}
			...
			{#if form.yeastType === 'sourdough' || !(form.bigaEnabled || form.poolishEnabled)}
				<label class="group text-ink-soft flex items-center gap-2 text-sm">
	...
	{#if uiMode.current === 'expert'}
		<details class="well text-ink-soft group min-w-0 p-4 text-xs">
			<summary
				...
				{#each INFO_SECTIONS as section (section.title)}
					<div class="min-w-0">
						...
						{#each section.parts as part, i (i)}
							...
							{:else}
								<pre
									class="border-hairline bg-raised text-ink mt-1 overflow-x-auto rounded-lg border px-2 py-1 font-mono text-[0.72rem]">{part.formula}</pre>
							{/if}
						{/each}
					</div>
				{/each}
