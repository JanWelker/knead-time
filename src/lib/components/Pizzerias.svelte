...
{#snippet rankingChips(rankings: Ranking[])}
	<ul class="flex flex-wrap gap-1 text-xs">
		{#each chronological(rankings) as r (`${r.year}-${r.list}`)}
			<li
				class="num ink-soft rounded-full bg-[var(--kt-inset)] px-2 py-0.5"
				title="{listLabel(r.list)} · {r.year}"
			>
				#{r.rank}
				<span class="ink-faint">{r.year}</span>
				<span class="ink-faint">{listLabel(r.list)}</span>
			</li>
		{/each}
	</ul>
{/snippet}
...
>
	...
	<ul class="flex flex-col gap-3 lg:hidden">
		{#each entries as entry (entry.recipeUrl)}
			<li class="rounded-lg border border-[var(--kt-line)] p-3">
				<div class="flex items-baseline justify-between gap-3">
					<span class="ink font-medium">
						{@render pizzeriaName(entry)}
					</span>
					<span class="ink-faint text-xs whitespace-nowrap">
				...
				<div class="mt-3 flex flex-wrap gap-2">
					<a
						...
						rel="external"
						class="btn-ember inline-flex items-center justify-center"
					>
				...
				<details class="mt-3 text-sm">
					<summary class="ink-faint cursor-pointer hover:text-[var(--kt-accent)]">
						{t.pizzerias.details_label}
					</summary>
					<RecipeSpecList inputs={entry.inputs} labels={t.pizzerias} />
					{#if entry.notes}
						<p class="ink-faint mt-2 text-xs italic">{entry.notes}</p>
					{/if}
				</details>
			</li>
		{/each}
	...
	<div class="hidden overflow-x-auto lg:block">
		<table class="w-full min-w-[840px] border-collapse text-left text-sm tabular-nums">
			<thead>
				<tr
					class="ink-faint border-b border-[var(--kt-line)] text-xs font-semibold tracking-[0.08em]"
				>
			...
			<tbody>
				{#each entries as entry (entry.recipeUrl)}
					<tr class="row-divider align-top">
						<td class="ink py-3 pr-3 font-medium">
							{@render pizzeriaName(entry)}
							{#if entry.notes}
								<div class="ink-faint mt-1 max-w-xs text-xs font-normal italic" title={entry.notes}>
									{entry.notes}
								</div>
							{/if}
						</td>
						<td class="ink-faint py-3 pr-3 whitespace-nowrap">
							{entry.city}, {entry.country}
						</td>
