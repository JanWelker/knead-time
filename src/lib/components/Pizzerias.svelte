...
{#snippet rankingChips(rankings: Ranking[])}
	<ul class="flex flex-wrap gap-1 text-xs">
		{#each chronological(rankings) as r (`${r.year}-${r.list}`)}
			<li
				class="bg-dough-100 text-dough-900 dark:bg-dough-900/50 dark:text-dough-100 rounded-full px-2 py-0.5 tabular-nums"
...
>
		...
		{#each entries as entry (entry.recipeUrl)}
			<li class="border-line rounded-lg border p-3">
				...
				<details class="mt-3 text-sm">
					<summary
						class="hover:text-tomato-600 dark:hover:text-tomato-300 text-ink-soft cursor-pointer"
					>
	...
	<div class="hidden overflow-x-auto lg:block">
		<table class="w-full min-w-[840px] border-collapse text-left text-sm tabular-nums">
			<thead>
				<tr class="border-line text-ink-soft border-b text-xs font-semibold">
