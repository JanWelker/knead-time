...
{#snippet rankingChips(rankings: Ranking[])}
	<ul class="flex flex-wrap gap-1 text-xs">
		{#each chronological(rankings) as r (`${r.year}-${r.list}`)}
			<li class="chip px-2 py-0.5 font-medium tabular-nums" title="{listLabel(r.list)} · {r.year}">
				#{r.rank}
...
>
	...
	<ul class="flex flex-col gap-3 lg:hidden">
		{#each entries as entry (entry.recipeUrl)}
			<li class="border-hairline rounded-xl border p-3">
				...
				<details class="mt-3 text-sm">
					<summary
						class="hover:text-tomato-600 dark:hover:text-tomato-300 text-ink-faint cursor-pointer"
					>
						{t.pizzerias.details_label}
					</summary>
					<RecipeSpecList inputs={entry.inputs} labels={t.pizzerias} />
					{#if entry.notes}
						<p class="text-ink-faint mt-2 text-xs italic">{entry.notes}</p>
					{/if}
				</details>
			</li>
		{/each}
	...
	<div class="hidden overflow-x-auto lg:block">
		<table class="w-full min-w-[840px] border-collapse text-left text-sm tabular-nums">
			<thead>
				<tr class="border-hairline text-ink-faint border-b text-xs font-medium">
			...
			<tbody>
				{#each entries as entry (entry.recipeUrl)}
					<tr class="row-divider align-top">
						<td class="text-ink py-3 pr-3 font-medium">
							{@render pizzeriaName(entry)}
							{#if entry.notes}
								<div
									class="text-ink-faint mt-1 max-w-xs text-xs font-normal italic"
									title={entry.notes}
								>
									{entry.notes}
								</div>
						...
						<td class="text-ink-faint py-3 pr-3 whitespace-nowrap">
							{entry.city}, {entry.country}
						</td>
