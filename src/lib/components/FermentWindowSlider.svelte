...
<div class="window-card">
	<div class="flex flex-wrap items-baseline justify-between gap-2">
		...
		<span class="num font-display ink text-2xl leading-none">{formatWindow(windowHours)}</span>
	</div>
	...
	{#if unreachableFromPct < 100}
		<div class="relative mx-2.5 mt-2 h-9" aria-hidden="true">
			...
			>
				...
				<span class="ink-faint text-[0.65rem] leading-tight whitespace-nowrap">
					{formatDateTime(form.readyBy, i18n.locale)}
				</span>
			</div>
			<svg
				class="absolute bottom-0 -translate-x-1/2 fill-[var(--kt-accent-solid)]"
	...
	{:else}
		<p class="ink-faint mt-1 text-xs">
			{t.form.readyBy}:
			<span class="ink-soft font-medium">
				{formatDateTime(form.readyBy, i18n.locale)}
			</span>
		</p>
	{/if}
	...
	<div class="relative mt-3 h-6">
		...
		<div
			class="absolute inset-x-2.5 top-1/2 h-2 -translate-y-1/2 overflow-hidden rounded-full bg-[var(--kt-line)]"
			aria-hidden="true"
		>
			{#if zones?.room}
				<div
					class="zone-room absolute inset-y-0"
					style="left:{axis(zones.room.min)}%;width:{axis(zones.room.max) - axis(zones.room.min)}%"
				></div>
			{/if}
			{#if zones?.cold}
				<div
					class="zone-cold absolute inset-y-0"
			...
			{#each stops as stop, i (stop)}
				{#if i > 0 && i < stops.length - 1}
					<div
						class="absolute inset-y-0 w-px bg-[var(--kt-panel)]/70"
			...
			{#if unreachableFromPct < 100}
				<div
					class="absolute inset-y-0 right-0 bg-[var(--kt-ground)]/90"
					style="left:{unreachableFromPct}%"
				></div>
			{/if}
		</div>
	...
	{#if idealPct !== null}
		<div class="relative mx-2.5 mt-1 h-9" aria-hidden="true">
			<svg
				class="absolute top-0 -translate-x-1/2 fill-[var(--kt-ink-faint)]"
				style="left:{idealPct}%"
				width="9"
				height="6"
				viewBox="0 0 10 6"
			...
			>
				<span class="ink-soft text-[0.65rem] leading-tight font-semibold whitespace-nowrap">
	...
	<div class="relative mx-2.5 mt-1 h-4" aria-hidden="true">
		{#each labelledStops as stop (stop)}
			<span
				class="num ink-faint absolute -translate-x-1/2 text-[0.65rem] {narrowLabelledStops.includes(
					stop
				)
					? ''
					: 'max-sm:hidden'}"
	...
	<div class="mt-2 flex items-start justify-between gap-2">
		...
		<p id="window-band" class="ink-faint text-xs">
			{#if band}
		...
		{#if ideal !== null && !atIdeal}
			<button type="button" class="btn-ember-sm shrink-0" onclick={() => form.repickWindow()}>
	...
	{#if reachableIndex >= 0 && band && sliderIndex >= reachableIndex && band.max > hoursUntilBake}
		<p class="ink-faint mt-2 text-xs">
	...
	<p id="window-benefit" class="ink-faint mt-2 text-xs">{benefit}</p>
</div>
