...
<div class="space-y-6">
		...
		<section>
			{#if section.heading}
				<header class="mb-2">
					...
					{#if section.help}
						<p class="ink-faint text-xs">{section.help}</p>
					{/if}
				</header>
			{/if}
				...
				<tbody>
					{#each section.rows as row (row.label)}
						<tr class="row-divider">
							<th class="ink-soft py-2 pr-3 text-left font-medium">
								{row.label}
								{#if row.hint}
									<span class="ink-faint text-xs font-normal">
										({row.hint})
									</span>
								{/if}
							</th>
							<td class="num ink py-2 text-right">{row.amount}</td>
						</tr>
					{/each}
					{#if section.total}
						<tr>
	...
	{#if needsFineScale(ingredients)}
		<p class="ink-faint text-xs italic">{t.ingredients.scale_hint}</p>
	{/if}
</div>
