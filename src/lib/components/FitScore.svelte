...
<details bind:this={detailsRef} class="group relative inline-block">
	<summary
		class="seal cursor-pointer list-none select-none"
		title={summaryTooltip}
		aria-label={interpolate(t.quality.fit_aria, { stars })}
	>
		<span class="text-accent-ink inline-block text-sm tracking-tight" aria-hidden="true">
			{starRow}
		</span>
		<span>{t.quality.fit_heading}</span>
	</summary>
	<div
		class="border-rule bg-sheet block-shadow absolute z-20 mt-2 max-w-sm rounded-[2px] border-2 p-3 text-sm"
	>
