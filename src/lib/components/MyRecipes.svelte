...
<details class="group" open={recipes.length > 0}>
	<summary class="flex cursor-pointer list-none items-start gap-2 select-none">
		...
		<header>
			...
			<p class="text-ink-soft mt-1 text-sm">{t.myRecipes.intro}</p>
		</header>
	</summary>
		...
		{:else}
			<ul class="flex flex-col gap-2">
				{#each recipes as recipe (recipe.name)}
					<li
						class="border-line flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3"
					>
						<div class="min-w-0">
						...
						<div class="flex shrink-0 items-center gap-2">
							...
							<button
								type="button"
								class="hover:text-tomato-600 dark:hover:text-tomato-300 text-ink-soft inline-block py-0.5 text-sm underline-offset-2 hover:underline"
