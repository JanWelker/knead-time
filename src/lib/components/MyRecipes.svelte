...
<details class="group" open={recipes.length > 0}>
	<summary class="flex cursor-pointer list-none items-start gap-2 select-none">
		...
		>
		<header>
			<h2 class="font-display ink text-2xl">
				{t.myRecipes.heading}
			</h2>
			<p class="ink-faint mt-1 text-sm">{t.myRecipes.intro}</p>
		</header>
	</summary>

	<div class="mt-4">
		{#if recipes.length === 0}
			<p class="ink-faint text-sm">{t.myRecipes.empty}</p>
		{:else}
			<ul class="flex flex-col gap-2">
				{#each recipes as recipe (recipe.name)}
					<li
						class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[var(--kt-line)] p-3"
					>
						<div class="min-w-0">
							<span class="ink block truncate font-medium">
								{recipe.name}
							</span>
						...
						<div class="flex shrink-0 items-center gap-2">
							...
							<button
								type="button"
								class="ink-faint inline-block py-0.5 text-sm underline-offset-2 hover:text-[var(--kt-accent)] hover:underline"
