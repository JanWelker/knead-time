...
import { openQuestion, openRecipe, region } from './helpers';
...
// utility can still win — but the family and the tracking come from that rule,
// and the whole type system is one grotesque separated by size and weight
// rather than a serif/sans pair. Changing a heading's LEVEL must not change its
// face, and the markup gives no hint that the two are coupled.
test('changing a heading level does not change its typeface', async ({ page }) => {
	...
	// One family for every role, so the day label and the step title agree.
	const day = await face(region(page, 'Schedule').locator('h3'));
	const step = await face(page.locator('main ol h4'));
	expect(day.font).toBe('"Helvetica Neue"');
	expect(step.font).toBe('"Helvetica Neue"');
	// The base rule's tightening, in pixels at the day label's 1.125 rem.
	expect(day.tracking).toBe('-0.36px');
});
