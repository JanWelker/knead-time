...
test('a utility can restyle a heading', async ({ page }) => {
	...
	expect((await face(card(page, 'Schedule').locator('h2'))).font).toBe('"Bodoni Moda"');
	// ...while the dateline, which asks for letterspaced capitals with nothing
	// but a component class, actually gets them.
...
test('every control keeps the focus ring, including the TRMNL uuid field', async ({ page }) => {
	...
	expect(await ring(page.locator('form input[type="number"]'))).toBe('rgb(160, 47, 26) solid 2px');
	expect(await ring(page.locator('form select'))).toBe('rgb(160, 47, 26) solid 2px');
	...
	expect(await ring(uuid)).toBe('rgb(160, 47, 26) solid 2px');
});
