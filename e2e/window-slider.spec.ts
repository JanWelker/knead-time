...
async function captionsInsideRail(page: Page) {
...
// The same rule, at the width that actually breaks it. The captions used to
// pivot to sit inside the rail only past a 12 % / 88 % threshold, and that pair
// was chosen against a desktop rail: on a phone the rail is ~300 px while the
// bake flag's caption is over half that, so a deadline at 85 % counted as
// "centred" and the caption hung off the side of the card. Desktop never showed
// it. The placement is measured now, in the units that decide it.
