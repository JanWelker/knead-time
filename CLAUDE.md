...
- Beginner shows readyBy/startAt/the window slider/pizzaCount/**flour**/mixingMethod plus a switch to expert; ball weight is fixed at the 280 g default and every other input keeps its default too. The flour preset select sits next to mixingMethod in both views (which flour is in the cupboard is something every baker knows, and it sets the fermentation band the schedule paints); the raw `flourW` number field behind it stays expert-only, since the presets already carry it. **The view mode only shapes the form — it never affects the schedule.**
- **Field order is one list, not two** — expert simply reveals more of it, so the two views never disagree about what comes first. `When`: readyBy (the anchor the whole app schedules back from) → startAt (the floor, with its "Now" button) → the window slider spanning them. `Recipe`: batch (pizzas, ball weight) → flour (preset, W) → baker's percentages (hydration, salt, oil, sugar) → mixing method → what leavens it (yeast, starter/pre-ferments, autolyse) → how and where it proofs (cold ball proof, room and fridge temperature).
