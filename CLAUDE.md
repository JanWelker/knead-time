...
- **Schedule.** Rendered as a **day-grouped vertical timeline** (`ScheduleTable.svelte`): consecutive steps fall under one date header, a rail threads the nodes (filled = baker-action step, hollow = waiting phase, dashed rail segment leaving a fermentation step), and the in-progress step gets a pulsing node + `Now` badge. **Step copy is split** (`stepCopy.ts`): `stepIngredients(step, msgs, schedule)` returns the weighed amounts as a structured `{amount, name}[]` list (rendered as a mini-table, never prose); `stepDescription` is method-only copy. Weighing steps (`prep`/`mix`/`preferment-mix`) carry an ingredient list; `divide` keeps `{n}`/`{weight}` interpolation; `mix` keeps `{water_temp}`. Day-two `prep`/`mix` omit yeast under a pre-ferment, and `prep` omits oil/sugar (weighed at `mix` then). **Separate `mix`/`prep` method templates per pre-ferment type** (`*_with_biga`, `*_with_poolish`, `prep_desc_with_preferment`): biga = stiff/no-knead day-one + day-two fold-in; poolish = whisk-and-pour. `preferment-mix` row spans the full duration; no separate `preferment-proof` step. **Source-timing badge**: when the form matches a `pizzeriaEntries` row and a step's computed duration falls outside the source range (±15 % tolerance), `ScheduleTable` renders the original value beneath the duration.
