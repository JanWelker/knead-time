...
- **Mixing method** (v4): `mixingMethod: 'hand' | 'machine'`. Machine = 15 min mix (the pre-v4 fixed value — old links keep exact step times), hand = 25 min. Hand also lowers the water-temp friction constant (5 °C vs the spiral mixer's 24 °C) in `idealMixWaterTempC(roomTempC, method)` and appends a `mix_technique_{hand,machine}` sentence to the mix copy. The fermentation solve is otherwise untouched (mix minutes shift the ferment budget deterministically).
...
- Defaults: 280 g / 70 % / 3 % salt / 0 % oil / 0 % sugar / machine mixing. No AVPN enforcement.
